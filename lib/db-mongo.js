...
function DB(credentials) {
  ...
  const COLLECTION_NAME = credentials.COLLECTION_NAME;
  const self = this;
  let db;
  ...
  self.init = () => {
    return new Promise((resolve, reject) => {
      ...
      if (!connectionString && credentials.MONGO_USERNAME && credentials.MONGO_PASSWORD && credentials.MONGO_HOSTS) {
        connectionString = `mongodb://${credentials.MONGO_USERNAME}:${credentials.MONGO_PASSWORD}@${credentials.MONGO_HOSTS}`;
      }

      if (!connectionString) {
        reject(new Error("Aucune information de  2 connexion MongoDB valide n'a été trouvée"));
        return;
      }
